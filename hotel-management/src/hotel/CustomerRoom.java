...
public class CustomerRoom extends JFrame {
	...
	public CustomerRoom() {
		...
		JLabel lblArizonaTea = new JLabel("Tên");
		lblArizonaTea.setFont(new Font("Roboto", Font.BOLD, 21));
		lblArizonaTea.setBounds(34, 299, 104, 26);
		contentPane.add(lblArizonaTea);
		
		JLabel lblPinaColada = new JLabel("Chọn nước");
		lblPinaColada.setFont(new Font("Roboto", Font.BOLD, 21));
		lblPinaColada.setBounds(22, 199, 183, 26);
		contentPane.add(lblPinaColada);
		
		name = new JTextField();
		name.setFont(new Font("Roboto", Font.BOLD, 21));
		...
		JLabel lblEspresso = new JLabel("SDT");
		lblEspresso.setFont(new Font("Roboto", Font.BOLD, 21));
		lblEspresso.setBounds(34, 414, 165, 21);
		contentPane.add(lblEspresso);
		
		addr = new JTextField();
		addr.setFont(new Font("Roboto", Font.BOLD, 20));
		...
		JLabel lblTotal = new JLabel("Chi phí món ăn");
		lblTotal.setFont(new Font("Roboto", Font.BOLD, 21));
		lblTotal.setBounds(33, 501, 192, 34);
		contentPane.add(lblTotal);
		
		meal = new JTextField();
		meal.setFont(new Font("Roboto", Font.PLAIN, 20));
		meal.setBounds(233, 501, 140, 34);
		contentPane.add(meal);
		meal.setColumns(10);
		
		JLabel lblCostOfDrinks = new JLabel("Chi phí đồ uống");
		lblCostOfDrinks.setFont(new Font("Roboto", Font.BOLD, 21));
		lblCostOfDrinks.setBounds(22, 561, 207, 26);
		contentPane.add(lblCostOfDrinks);
		
		drink = new JTextField();
		drink.setFont(new Font("Roboto", Font.PLAIN, 20));
		drink.setBounds(233, 557, 140, 34);
		contentPane.add(drink);
		drink.setColumns(10);
		
		JLabel lblSelectDish = new JLabel("Chọn món ăn");
		lblSelectDish.setForeground(new Color(255, 255, 255));
		lblSelectDish.setFont(new Font("Roboto", Font.BOLD, 20));
		lblSelectDish.setBounds(34, 128, 181, 22);
		contentPane.add(lblSelectDish);
		
		//JComboBox comboBox_Dish = new JComboBox();
		comboBox_Dish.setFont(new Font("Roboto", Font.BOLD, 21));
		comboBox_Dish.setBounds(213, 122, 281, 34);
		contentPane.add(comboBox_Dish);
		
		//JComboBox comboBox_Drink = new JComboBox();
		comboBox_Drink.setFont(new Font("Roboto", Font.BOLD, 21));
		comboBox_Drink.setBounds(215, 195, 268, 34);
		contentPane.add(comboBox_Drink);
		
		JButton btnGenerateReceipt = new JButton("Tạo biên lai");
		btnGenerateReceipt.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				calculateAmount();
				area.setText("***********************************************\n");
				area.setText(area.getText()+"***               Đang xuất hóa đơn, vui lòng đợi!        	    ***\n");
				area.setText(area.getText()+"***********************************************\n\n");
				area.setText(area.getText()+"Thời gian     : "+sf.format(dt)+"\n\n");
				area.setText(area.getText()+"Tên     :    "+name.getText()+"\n\n");
				area.setText(area.getText()+"Địa chỉ  :    "+addr.getText()+"\n\n");
				area.setText(area.getText()+"SDT  :    "+phone.getText()+"\n\n");
				area.setText(area.getText()+"Món ăn đã gọi  : "+comboBox_Dish.getSelectedItem()+"\n\n");
				area.setText(area.getText()+"Đồ uống đã gọi  : "+comboBox_Drink.getSelectedItem()+"\n\n");
				area.setText(area.getText()+"Tổng cộng   :   "+totalA.getText()+"\n\n");
		...
		btnGenerateReceipt.setFont(new Font("Roboto", Font.BOLD, 21));
		btnGenerateReceipt.setBounds(257, 673, 281, 39);
		contentPane.add(btnGenerateReceipt);
		
		JButton btnTotal = new JButton("Tổng cộng");
		btnTotal.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(name.getText().equals(""))
				{   a1.setVisible(true);
					throw new MyException("Vui lòng điền đầy đủ thông tin");
		...
		btnTotal.setFont(new Font("Roboto", Font.BOLD, 21));
		btnTotal.setBounds(45, 673, 154, 39);
		contentPane.add(btnTotal);
		
		JButton btnCheckout = new JButton("Kiểm tra");
		...
		btnCheckout.setFont(new Font("Roboto", Font.BOLD, 21));
		...
		totalA.setFont(new Font("Roboto", Font.BOLD, 20));
		totalA.setBounds(231, 613, 142, 34);
		contentPane.add(totalA);
		totalA.setColumns(10);
		
		JLabel lblTotalAmount = new JLabel("Tổng cộng");
		lblTotalAmount.setFont(new Font("Roboto", Font.BOLD, 20));
		...
		btnBack.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(flag==0)
					JOptionPane.showMessageDialog(null, "Đầu tiên bạn cần tạo biên lai");
				else {
		...
		btnBack.setFont(new Font("Roboto", Font.BOLD, 20));
    ...
    ResultSet result1 = null;
		try {
			...
			String sql = "SELECT * FROM NhaHang where LoaiMon='Đồ ăn' ORDER BY TenMon ASC ";
			...
			String sql2 = "SELECT * FROM NhaHang where LoaiMon='Thức uống' ORDER BY TenMon ASC";
			 ps1=conn.prepareStatement(sql2);
			result1= ps1.executeQuery();
			while(result1.next())
			{
				String Dname= result1.getString("TenMon");
	...
	public void FillDishes()
	{
		...
		Connection conn=connect.getConnection();
		 ...
		 model.addColumn("Tên món ăn");
			...
			try {
				String query = "SELECT * FROM NhaHang where LoaiMon='Đồ ăn'";
	...
	public void FillDrinks()
	{
		...
		Connection conn=connect.getConnection();
		 ...
		 model2.addColumn("Tên nước");
			model2.addColumn("Giá");
			
			try {
				String query = "SELECT * FROM NhaHang where LoaiMon='Thức uống'";
				Statement st= conn.createStatement();
				ResultSet rs= st.executeQuery(query);
				while(rs.next())
				{
	...
	public void calculateAmount()
	{
		...
		try {
			String sql="SELECT Gia FROM NhaHang WHERE TenMon = ?";
		 ...
		 PreparedStatement ps = null;
		    ResultSet result = null;
				try {
			    		...
			    		GetConnection connect=new GetConnection();
			    		Connection conn=connect.getConnection();
			        	 ps=conn.prepareStatement("INSERT INTO KhachHangCuaNhaHang (TenKhachHang,DiaChi,SDT,DoAn,ThucUong,Gia,TrangThai) VALUES (?,?,?,?,?,?,?)");
	...
	public void checkOut()
	{
	    ...
	    ResultSet result = null;
			try {
		    		...
		    		GetConnection connect=new GetConnection();
		    		Connection conn=connect.getConnection();
		        	 ps=conn.prepareStatement("UPDATE KhachHangCuaNhaHang SET TrangThai = 1 where TenKhachHang=?");
		        	 ps.setString(1,name.getText());

...
public class AddDishes extends JFrame {
	...
	public AddDishes() {
		...
		JLabel lblDishName = new JLabel("Tên món");
		lblDishName.setFont(new Font("Roboto", Font.BOLD, 20));
		lblDishName.setBounds(33, 211, 155, 22);
		contentPane.add(lblDishName);
		
		JLabel lblD = new JLabel("Giá");
		lblD.setFont(new Font("Roboto", Font.BOLD, 20));
		lblD.setBounds(33, 284, 155, 27);
		contentPane.add(lblD);
		
		JLabel lblDishType = new JLabel("Loại món");
		lblDishType.setFont(new Font("Roboto", Font.BOLD, 20));
		lblDishType.setBounds(33, 353, 155, 27);
		contentPane.add(lblDishType);
		
		d1 = new JTextField();
		d1.setFont(new Font("Roboto", Font.BOLD, 20));
		d1.setBounds(182, 207, 232, 30);
		contentPane.add(d1);
		d1.setColumns(10);
		
		d2 = new JTextField();
		d2.setFont(new Font("Roboto", Font.BOLD, 20));
		d2.setBounds(182, 282, 232, 30);
		contentPane.add(d2);
		d2.setColumns(10);
		
		d3 = new JTextField();
		d3.setFont(new Font("Roboto", Font.BOLD, 20));
		...
		btnAddDish.setFont(new Font("Roboto", Font.BOLD, 20));
		btnAddDish.setBounds(45, 486, 176, 53);
		contentPane.add(btnAddDish);
		
		JButton btnDeleteDish = new JButton("Xóa món");
		...
		btnDeleteDish.setFont(new Font("Roboto", Font.BOLD, 20));
		btnDeleteDish.setBounds(245, 486, 221, 53);
		contentPane.add(btnDeleteDish);
		
		JButton btnUpdateDish = new JButton("Cập nhật món");
		...
		btnUpdateDish.setFont(new Font("Roboto", Font.BOLD, 20));
	...
	public void displayDishes()
	{
		...
		model.addColumn("Thứ tự món");
		model.addColumn("Tên món");
		model.addColumn("Loại món");
		model.addColumn("Giá");
		
		try {
			String query = "SELECT * FROM NhaHang";
		 ...
		 PreparedStatement ps = null;
		    ResultSet result = null;
				try {
			    		...
			    		GetConnection connect=new GetConnection();
			    		Connection conn=connect.getConnection();
			        	 ps=conn.prepareStatement("INSERT INTO NhaHang(TenMon,Gia,LoaiMon) VALUES (?,?,?)");
			             ...
			             {
			            	 JOptionPane.showMessageDialog(null, "Món mới thêm");
			             }
		    ...
		    ResultSet result = null;
				try {
					...
					System.out.println(no);
			    		GetConnection connect=new GetConnection();
			    		Connection conn=connect.getConnection();
			        	 ps=conn.prepareStatement("UPDATE NhaHang SET TenMon=?, Gia= ?, LoaiMon=? where IDMon=?");
		                 ps.setString(1,d1.getText());
			             ps.setString(2,d2.getText());
			             ps.setString(3,d3.getText());
			             ps.setString(4, no);
			             ...
			             if(ps.executeUpdate()>0)
			             {
			            	 JOptionPane.showMessageDialog(null, "  Cập nhật món ");
			             }
	    ...
	    ResultSet result = null;
			try { 
				...
				System.out.println(no);
		    		GetConnection connect=new GetConnection();
		    		Connection conn=connect.getConnection();
		        	 ps=conn.prepareStatement("DELETE FROM NhaHang  where IDMon=?");
		             ...
		             if(ps.executeUpdate()>0)
		             {
		            	 JOptionPane.showMessageDialog(null, " Xóa món ");
		             }

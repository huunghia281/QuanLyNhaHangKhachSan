...
public class CustomerRestaurant extends JFrame {
	...
	public CustomerRestaurant() {
		...
		JLabel lblPhoneNum = new JLabel("SDT ");
		lblPhoneNum.setFont(new Font("Roboto", Font.BOLD, 20));
		lblPhoneNum.setBounds(25, 229, 164, 25);
		contentPane.add(lblPhoneNum);
		
		JLabel lblRoomType = new JLabel("Loại phòng");
		lblRoomType.setFont(new Font("Roboto", Font.BOLD, 20));
		lblRoomType.setBounds(32, 343, 134, 32);
		contentPane.add(lblRoomType);
		
		JLabel lblBedType = new JLabel("Loại giường");
		lblBedType.setFont(new Font("Roboto", Font.BOLD, 20));
		...
		btnGenerateReceipt = new JButton("Tạo biên lai");
		btnGenerateReceipt.setFont(new Font("Roboto", Font.BOLD, 20));
		btnGenerateReceipt.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				calculateTotal();
				textArea.setText("**************************************************************************\n");
				textArea.setText(textArea.getText()+"******      	    Đang xuất hóa đơn, vui lòng đợi!      	     ******\n");
				textArea.setText(textArea.getText()+"*************************************************************************\n\n");
				textArea.setText(textArea.getText()+"Thời gian     : "+sf.format(dt)+"\n\n");

				textArea.setText(textArea.getText()+"Tên     :    "+name.getText()+"\n\n");
				textArea.setText(textArea.getText()+"Địa chỉ  :    "+addr.getText()+"\n\n");
				textArea.setText(textArea.getText()+"SDT  :    "+phone.getText()+"\n\n");
				textArea.setText(textArea.getText()+"Loại phòng  : "+roomtype.getText()+"\n\n");
				textArea.setText(textArea.getText()+"Loại giường  : "+bedtype.getText()+"\n\n");
				textArea.setText(textArea.getText()+"Tổng số tiền   :   "+tot.getText()+"\n\n");
		...
		btnBack.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (flag==0)
					JOptionPane.showMessageDialog(null, "Trước tiên bạn cần phải tạo biên lai");
				else {
		...
		btnCheckout = new JButton("Kiểm tra");
		btnCheckout.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				checkOut();
			}
		});
		btnCheckout.setFont(new Font("Roboto", Font.BOLD, 20));
		btnCheckout.setBounds(726, 603, 184, 38);
		contentPane.add(btnCheckout);
		
		days = new JTextField();
		days.setFont(new Font("Roboto", Font.BOLD, 20));
		days.setBounds(201, 507, 197, 32);
		contentPane.add(days);
		days.setColumns(10);
		
		JLabel lblNoOfDays = new JLabel("Số ngày thuê");
		lblNoOfDays.setFont(new Font("Roboto", Font.BOLD, 20));
		lblNoOfDays.setBounds(32, 511, 147, 25);
		contentPane.add(lblNoOfDays);
		
		JButton btnTotal = new JButton("Tổng cộng");
		btnTotal.setFont(new Font("Roboto", Font.BOLD, 15));
		...
		lblTotal.setFont(new Font("Roboto", Font.BOLD, 20));
		lblTotal.setBounds(32, 556, 107, 25);
		contentPane.add(lblTotal);
		
		tot = new JTextField();
		tot.setFont(new Font("Roboto", Font.BOLD, 20));
		...
		lblRoomNo = new JLabel("Phòng số");
		lblRoomNo.setFont(new Font("Roboto", Font.BOLD, 20));
		lblRoomNo.setBounds(32, 286, 147, 29);
		contentPane.add(lblRoomNo);
		
		rno = new JTextField();
		rno.setFont(new Font("Roboto", Font.BOLD, 20));
		 ...
		 PreparedStatement ps = null;
		    ResultSet result = null;
				try {
			    		...
			    		GetConnection connect=new GetConnection();
			    		Connection conn=connect.getConnection();
			        	 ps=conn.prepareStatement("INSERT INTO KhachHangThuePhong(TenKhachHang,DiaChi,SDT,LoaiPhong,LoaiGiuong,Gia,TrangThai) VALUES (?,?,?,?,?,?,?)");
			             ...
			             {
			            	 JOptionPane.showMessageDialog(null, "Đã thêm khách hàng mời");
			             }
	...
	public void checkOut()
	{
	    ...
	    ResultSet result = null;
			try {
		    		...
		    		GetConnection connect=new GetConnection();
		    		Connection conn=connect.getConnection();
		        	 ps=conn.prepareStatement("UPDATE KhachHangThuePhong SET TrangThai = 1 where TenKhachHang=?");
		        	 ps.setString(1,name.getText());

...
public class AddRooms extends JFrame {
	...
	public AddRooms() {
		...
		JButton btnAddRooms = new JButton("Thêm phòng");
		...
		btnAddRooms.setFont(new Font("Roboto", Font.BOLD, 23));
		btnAddRooms.setBounds(12, 456, 225, 50);
		contentPane.add(btnAddRooms);
		
		JButton btnAddDish = new JButton("Cập nhật phòng");
		...
		btnAddDish.setFont(new Font("Roboto", Font.BOLD, 23));
		btnAddDish.setBounds(518, 456, 270, 50);
		contentPane.add(btnAddDish);
		
		JButton btnDleteRooms = new JButton("Xóa phòng");
		btnDleteRooms.setIcon(new ImageIcon("images\\iconfinder_f-cross_256_282471 (1).png"));
		btnDleteRooms.setFont(new Font("Roboto", Font.BOLD, 23));
		...
		JLabel lblNewLabel = new JLabel("Mới");
		 ...
		 PreparedStatement ps = null;
		    ResultSet result = null;
				try {
			    		...
			    		GetConnection connect=new GetConnection();
			    		Connection conn=connect.getConnection();
			        	 ps=conn.prepareStatement("INSERT INTO Phong(LoaiPhong,LoaiGiuong,Gia) VALUES (?,?,?)");
			             ...
			             {
			            	 JOptionPane.showMessageDialog(null, "Đã thêm phòng mới vào");
			             }
		    ...
		    ResultSet result = null;
				try {
					...
					System.out.println(no);
			    		GetConnection connect=new GetConnection();
			    		Connection conn=connect.getConnection();
			        	 ps=conn.prepareStatement("UPDATE Phong SET LoaiPhong=?, LoaiGiuong= ?, Gia=? where IDPhong=?");
		                 ps.setString(1,r1.getText());
			             ps.setString(2,r2.getText());
			             ps.setString(3,r3.getText());
			             ps.setString(4, no);
			             ...
			             if(ps.executeUpdate()>0)
			             {
			            	 JOptionPane.showMessageDialog(null, " Phòng đã được cập nhật ");
			             }
	    ...
	    ResultSet result = null;
			try {
				...
				System.out.println(no);
		    		GetConnection connect=new GetConnection();
		    		Connection conn=connect.getConnection();
		        	 ps=conn.prepareStatement("DELETE FROM Phong  where IDPhong=?");
